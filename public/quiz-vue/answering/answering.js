...
<div class="card" style="margin-bottom:80px;" v-show="Q.step == 1">

<div class="card-header"><b>Question {{Q.id+1}}: {{Q.q_id}}</b></div>
<div class="card-body">
    <question-viewing v-bind:pdfs="Q.pdf" v-bind:images="Q.images"  v-show="gradingSystem.checkQuestionHasResources(Q)"></question-viewing>
...
<div class="card-footer">
    <input type="submit" class="btn btn-secondary" style="float:right;" value="Submit" v-on:click="Q.step = Q.step + 1;gradingSystem.marking(Q.id);"></input>
</div>
...
<!-- marking -->
<div v-for="Q in questions">
<div class="card" style="margin-bottom:80px" v-show="Q.self_mark && Q.step == 2">
...
<div class="card-body">
    ...
    <div v-show="!gradingSystem.checkQuestionHasResources(Q)"></div>
    
	<question-viewing v-bind:pdfs="Q.anspdf" v-bind:images="Q.answerimages"  v-show="gradingSystem.checkQuestionHasAnswerResources(Q)">
	<h3>Answers:</h3>
	</question-viewing>
...
<div class="card-footer">
    ...
    <input type="reset" class="btn btn-secondary" style="float:left;" value="Reset" v-on:click="gradingSystem.process([Q]);"/>
</div>
</div>
</div>
<!-- results -->
<div class="card" style="margin-bottom:80px" v-show="gradingSystem.checkMarkedComplete(questions)">
...
<div class="card-body">
    ...
    <table class="table">
    <tr><th v-show="gradingSystem.checkQuestionHasResources(Q)">Images</th><th>Answer</th><th width="40%">Correct Answer</th></tr>
        <tr>
            <td width="20%" v-show="gradingSystem.checkQuestionHasResources(Q)">
            ...
            <td width="20%"  v-show="gradingSystem.checkQuestionHasAnswerResources(Q)">
...
Vue.component('pdf-viewer', {
  props: ['pdfs'],
  template:
        `<span><object v-show="pdfs != null" v-for="pdf in pdfs"
...
Vue.component('question-viewing', {
	...
	<div class="question-viewing"> 
        ...
        <a v-show="images != null" v-bind:href="image" target="_blank"  v-for="image in images"><img v-bind:src="image" alt="No related image of question here"/></a>
    </div> 
	`
});
...
Vue.component('resources-rundown', {
	...
	template: `
	<div>
	<div v-show="pdfs != null" v-for="(pdf,index) in pdfs"><a v-bind:href="'http://docs.google.com/gview?url='+pdf+'&embedded=true'" target="_blank" >PDF Link {{index+1}}</a></div>
    <a v-show="images != null" v-bind:href="image" target="_blank"  v-for="image in images"><img v-bind:src="image" alt="No related image of question here"/></a>
	</div>
	`
});
